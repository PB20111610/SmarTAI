"""
Concept question correction node implementation.
"""
import structlog
...
def parse_llm_json_response(response_text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Extract hits
        hits_match = re.search(r'"hits"\s*:\s*$(.*?)$', response_text)
        if hits_match:
            try:
                hits_array = json.loads("[" + hits_match.group(1) + "]")
                llm_response["hits"] = hits_array
            except json.JSONDecodeError:
                # If that fails, try to extract individual strings
                hits_strings = re.findall(r'"([^"]*)"', hits_match.group(1))
                llm_response["hits"] = hits_strings
        else:
            llm_response["hits"] = []
...
def concept_node(answer_unit: Dict[str, Any], rubric: str, max_score: float = 10.0) -> Correction:
    """
    Concept question correction node.
    
    Args:
        answer_unit: The answer unit containing the student's answer
    ...
    """
    logger.info("concept_node_start", q_id=answer_unit.get("q_id", "unknown"))
    
    # Step 1: Extract keywords from knowledge base (mock implementation)
    # In a real implementation, this would use some NLP technique or LLM
    keywords = [f"知识点{i}" for i in range(5)]  # Mock keywords
    ...
    try:
        ...
        logger.info("concept_prompt_prepared", prompt=prompt[:100] + "..." if len(prompt) > 100 else prompt)
        ...
        try:
            ...
            comment = llm_response.get("comment", f"计算过程包含 {len(step_scores)} 个步骤")
            response_max_score = llm_response.get("max_score", max_score)  # Use AI response max_score if available
            hits = llm_response.get("hits", keywords[:3])  # First 3 keywords as hits
            ...
            try:
                correction = Correction(
                    q_id=answer_unit.get("q_id", "unknown"),
                    type="concept",
                    score=total_score,
                ...
                logger.error("correction_creation_failed", error=str(correction_error), 
                           q_id=answer_unit.get("q_id", "unknown"), type="concept", score=total_score, 
                           max_score=max_score, confidence=overall_confidence,
                           comment=comment,
                           steps_count=len(step_scores))
                raise
            
            logger.info("concept_node_complete", q_id=answer_unit.get("q_id", "unknown"), score=correction.score, steps_count=len(correction.steps))
            return correction
        except Exception as e:
            ...
            correction = Correction(
                ...
                hits=keywords[:2]  # First 2 keywords as hits
            )
            return correction
...
def process_concept_from_files(input_file: str, rubric_file: str, output_file: str, max_score: float = 10.0):
    """
    Process a concept question from input files and write results to output file.
    ...
    logger.info("concept_processing_complete", input_file=input_file, output_file=output_file)
    return correction


if __name__ == "__main__":
    # Set up argument parser for file-based processing
    parser = argparse.ArgumentParser(description="Process concept question from files")
    ...
    try:
        correction = process_concept_from_files(args.input, args.rubric, args.output, args.max_score)
        print(f"Concept question processed successfully. Results written to {args.output}")
        ...
        print(f"Hits: {correction.hits}")
    except Exception as e:
        logger.error("concept_processing_failed", error=str(e), exc_info=True)
        print(f"Error processing concept question: {e}")
        exit(1)