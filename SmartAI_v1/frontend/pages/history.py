...
if "jobs" not in st.session_state or not st.session_state.jobs:
    ...
    for job_id, task_info in list(st.session_state.jobs.items()):
        ...
        if status == "completed":
            ...
            if cols[3].button("查看结果", key=f"view_{job_id}"):
                # Store the selected job ID in session state and switch to results page
                st.session_state.selected_job_id = job_id
                st.switch_page("pages/grade_results.py")
        else:
            # If task is not completed, show the remove button
