...
def render_students_dashboard():
    ...
    students_data = st.session_state.processed_data.get('students', {})
    questions_data = st.session_state.prob_data.get('questions', [])
    
    if not students_data or not questions_data:
    ...
    for student_id, student_info in students_data.items():
        row = {'学生ID': student_id}
        answers_map = {ans.get('question_id'): ans for ans in student_info.get('answers', [])}
    ...
    if dashboard_data:
        df = pd.DataFrame(dashboard_data).set_index('学生ID')
