...
def render_question_overview():
    ...
    questions = st.session_state.prob_data.get('questions', [])
        ...
        with st.container(border=True):
            ...
            if st.session_state[edit_stem_key]:
                ...
                with col1:
                    if st.button("✅ 保存题干", key=f"save_stem_btn_{q_id}", type="primary", use_container_width=True):
                        st.session_state.prob_data['questions'][i]['stem'] = new_stem
                        st.session_state[edit_stem_key] = False
                        st.rerun()
                with col2:
            ...
            # --- 模式2: 编辑评分标准 ---
                ...
                with col1:
                    if st.button("✅ 保存标准", key=f"save_criterion_btn_{q_id}", type="primary", use_container_width=True):
                        st.session_state.prob_data['questions'][i]['criterion'] = new_criterion
                        st.session_state[edit_criterion_key] = False
                        st.rerun()
                with col2:
            ...
            # --- 模式3: 正常显示 ---
                ...
                with col1:
                    ...
                    # 如果类型发生变化，直接更新
                    if new_type != st.session_state.prob_data['questions'][i]['type']:
                        st.session_state.prob_data['questions'][i]['type'] = new_type
                        st.rerun()
